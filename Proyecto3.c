...
void guardarArbol(FILE* file, MinHeapNode* root) {
    ...
    huffmanTreeChildren[currentIndex][0] = -1;
    huffmanTreeChildren[currentIndex][1] = -1;
    ...
    if (root->left) {
        huffmanTreeChildren[currentIndex][0] = cont;  
        guardarArbol(file, root->left);               
    }

    
    if (root->right) {
        huffmanTreeChildren[currentIndex][1] = cont; 
